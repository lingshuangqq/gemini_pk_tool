...
PRICING_GCP["gemini-3.6-flash"] = {"in": 0.075 / 1000000.0, "out": 0.30 / 1000000.0}
PRICING_API["gemini-3.6-flash"] = {"in": 0.075 / 1000000.0, "out": 0.30 / 1000000.0}

class Gemini36FlashTester(BaseModelTester):
    """
    Plugin tester for gemini-3.6-flash.
    """
    ...
    def get_traffic_profiles(cls):
        return {
            "standard": {"name": "标准巡检 (3.6 Flash Standard)", "trials": 5, "delay": 1.5, "desc": "常规同频延迟与通信比对测试"},
            "concurrency_stress": {"name": "高频并发压力 (3.6 Flash Concurrency Stress)", "trials": 25, "delay": 0.3, "desc": "模拟高负载高突发情况下的并发测试"},
            "heavy_burst": {"name": "极限频发冲击 (3.6 Flash Heavy Burst)", "trials": 50, "delay": 0.1, "desc": "极短延迟下的吞吐爆破压测"}
        }

    def get_default_prompt(self):
        return "Explain the main architectural capabilities of Gemini 3.6 Flash in exactly 40 words."
...
export_class = Gemini36FlashTester
